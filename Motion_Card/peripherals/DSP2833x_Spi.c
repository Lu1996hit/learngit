...
//	For SPIBRR = 0, 1, or 2:	SPI Baud Rate = LSPCLK / 4
	...
	//define SPI Baud Rate = LSPCLK / 4
	Spi->RegsAddr->SPIBRR	= 0;
...
//	Spi->RegsAddr->SPICCR.all =0x0007;   	// Reset on, rising edge, 8-bit char bits
	Spi->RegsAddr->SPICCR.all =0x000F;   	// Reset on, rising edge, 16-bit char bits
...
// 配置SPI的相序
	Spi->RegsAddr->SPICCR.bit.CLKPOLARITY = 0;
	Spi->RegsAddr->SPICTL.bit.CLK_PHASE = 1;
